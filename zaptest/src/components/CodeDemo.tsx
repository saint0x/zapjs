...
const codeExamples = [
  {
    ...
    language: 'typescript',
    code: `import { server, User, ApiError } from '../generated/server';
...
export function UserList() {
  ...
  useEffect(() => {
    async function fetchUsers() {
      const result = await server.default.listUsers({
        limit: 10,
        offset: 0
      });
...
pub fn list_users(limit: u32, offset: u32) -> Result<ListUsersResponse, ApiError> {
  ...
  },
  {
    ...
    icon: Sparkles,
    filename: 'src/generated/server.ts',
    language: 'typescript',
    code: `// Auto-generated from Rust - DO NOT EDIT
import { rpcCall } from './rpc-client';
import type { User, ListUsersResponse, ApiError } from './types';
...
export const server = {
  default: {
    async listUsers(params: { limit: number, offset: number }): Promise<ListUsersResponse | ApiError> {
      return rpcCall<ListUsersResponse | ApiError>('list_users', {
        limit: params.limit,
    ...
    async createUser(params: { name: string, email: string, role: string }): Promise<User | ApiError> {
      return rpcCall<User | ApiError>('create_user', params);
    },
  },
} as const;`,
  },
